...
function postPollsIdRepliesIdStars(options) {
    ...
    var path = '/polls/' + options.pollId + '/replies/' + options.replyId + '/stars';
    postAPI(path, {}, done, fail);
}
...
function postPollsIdRepliesIdStarsUI(options) {
...
function renderReply(options, html) {
    ...
    var postId = $(html).attr('data-post-id');

    if ($('div.post-reply-' + postId).length == 0) {
...
function incrementVote(pollId, optionId) {
    pollOptions[pollId][optionId]['votes'] += 1;
}
...
function initPubnub(pollId) {
    var pubnub = PUBNUB.init({
        // publish_key   : 'PUBLISH_KEY_HERE',
        subscribe_key : 'sub-c-39fb4ed2-8625-11e3-bf5b-02ee2ddab7fe',
        ssl           : true,
